...
function jsonResponse(Response $response, $data, int $status = 200): Response {
    ...
    if ($json === false) {
        // Fallback en caso de error al codificar JSON
...
// Función para generar UUID (si no lo hace la base de datos)
...
$app->get('/api/albaranes[/]', function (Request $request, Response $response) use ($servername, $username, $password, $dbname) {
    try {
        ...
        $stmt = $conn->prepare("SELECT kalbaran, fecha_dtm, kalmacen, ktipodeprecio, comentario_str 
                               FROM tblalbaran 
...
$app->get('/api/gastos/descargararchivo/{id}', function (Request $request, Response $response, array $args) use ($servername, $username, $password, $dbname) {
    try {
        ...
        $stmt->bind_param("s", $args['id'], $kagricultor);
        $stmt->execute();
        $stmt->store_result();
        if ($stmt->num_rows === 0) {
            $stmt->close();
